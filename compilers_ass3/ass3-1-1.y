...
id_arr:				mult_star IDENTIFIER br_dimlist	
													{
														$$ = new variable;
														$$->name = *$2;
														$$->type = 1;
...
asg:				lhs EQUALS exp				
													{
														...
														{	
															...
															else if((*($1->place->dimlist)).size() == 0)
															{
																quadruple q;
																q.s1 = "";
...
br_elist:			br_elist RIGHT_CLOSED_BRACKET LEFT_CLOSED_BRACKET exp
													{
														...
														*($$->place) = *($1->place);
														...
														if(found)
														{
															if((*(vn->dimlist)).size() >= $$->dimnum)
																num_elem = (*(vn->dimlist))[$$->dimnum];
															else
															{
														...
														else
														{
															...
															if(found)
															{
																if((*(pn->dimlist)).size() >= $$->dimnum)
																	num_elem = (*(pn->dimlist))[$$->dimnum];
																else
																{
																	error("Dimension given high.");
																}
...
f_exp:				NUMBER_INT				
				...
				|	mult_star br_elist RIGHT_CLOSED_BRACKET
													{
														...
														if(!found)
														{
															...
															{
																$$->eletype = ERRORTYPE;
																error("Identifier " + $2->place->name + " not declared");
															}
															else
															{
																$$->eletype = pn->eletype;
														...
														if($$->eletype != ERRORTYPE)
														{
															...
															q.oprnd_1 = $$->name;
															q.s2 = "=";
															q.oprnd_2 = $2->place->name;
															q.s3 = "[";
															q.oprnd_3 = temp->name;
															q.s4 = "]";
															gen(q);
														}
														if($2->offset->inside != $2->place->dimlist->size())
															error("Mismatch in dimension");
														else
															$$->type = 0;
													}
...
{
	param_no = param_no - 1;
	if((global_func[call_func_ptr].param_list[param_no].eletype != var->eletype) || (global_func[call_func_ptr].param_list[param_no].type != var->type))
	{
		ok = false;
		return;
	}
	...
	if(global_func[call_func_ptr].param_list[param_no].struct_name == var->struct_name)
	{
		ok = true;
	}
	else
