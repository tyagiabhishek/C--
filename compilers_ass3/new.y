%{
	...
	void conv_char_quad(variable *a,variable *operand1);
														...
														{	
															...
															else
															{
																if($1->place->eletype != $3->eletype)
																{
																	...
																	else if($1->place->eletype == BOOL)
																		conv_bool_quad($3,operand1);
																	else if($1->place->eletype == CHA)
																		conv_char_quad($3,operand1);
...
br_elist:			br_elist RIGHT_CLOSED_BRACKET LEFT_CLOSED_BRACKET exp
													{
														...
														*($$->place) = *($1->place);
														...
														if($$->place->eletype != ERRORTYPE)
														{
															...
															int temp1 = (*$$->place->dimlist)[$$->offset->inside - 1];
															// quadruple q;
															q.s1 = "if";
															q.oprnd_1 = $$->offset->name;
															q.s2 = "<";
															q.oprnd_2 = to_string(temp1);
															q.s3 = "goto L";
															q.oprnd_3 = to_string(nextquad + 3);
															q.s4 = "";
															gen(q);
															q.s1 = "error('array overflow in (";
															q.oprnd_1 = to_string($$->offset->inside);
															q.s2 = ")')";
															q.oprnd_2 = "";
															q.s3 = "";
															q.oprnd_3 = "";
				...
				|	IDENTIFIER LEFT_CLOSED_BRACKET exp
													{
														...
														else if((*($$->place->dimlist)).size() > 0)
														{
															*$$->offset = *$3;
															// $$->offset->type = $3->type;
															// $$->offset->eletype = $3->eletype;
															// $$->offset->dimlist = new vector<int>;
															// *($$->offset->dimlist) = *($3->dimlist);
															// ($$->offset->level) = $3->level;
															// ($$->offset->struct_name) = $3->struct_name;
															// ($$->offset->inside) = $3->inside;
															newtemp($$->offset);
															temp1 = (*$$->place->dimlist)[0];
															quadruple q;
															q.s1 = "";
															q.oprnd_1 = $$->offset->name;
															q.s2 = "=";
